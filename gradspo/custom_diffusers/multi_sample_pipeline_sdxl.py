...
from .ddim_seperate import ddim_step_fetch_x0, ddim_step_fetch_x_t_1, _get_variance
...
def multi_sample_pipeline_sdxl(
    ...
    **kwargs,
        ...
        for i, t in enumerate(timesteps):
            ...
            if (i >= divert_start_step and inner_step_left == 0) or i == len(timesteps) - 1:
                ...
                if i == divert_start_step:
                    ...
                    prev_latents, noise_temp = ddim_step_fetch_x_t_1(
                        self.scheduler,
                        dtype=latents.dtype,
                    ...
                    noise_added_cache.append(noise_temp)
                    ...
                    idx_max = preference_scores.max(dim=0)[1]
                    idx_range = torch.arange(preference_scores.shape[1]).to(preference_scores.device)
                    idx_min = preference_scores.min(dim=0)[1]
                    max_noise = noise_added_cache[0][idx_max, idx_range]
                    min_noise = noise_added_cache[0][idx_min, idx_range]
                    win_noise = (max_noise-min_noise)/2
                    lose_noise = -win_noise.to(latents.dtype)
                    batch_temp = win_noise.shape[0]
                    variance = _get_variance(self.scheduler, timesteps[i-1], timesteps[i])
                    eta = extra_step_kwargs['eta']
                    noise_sam = torch.cat([win_noise.unsqueeze(0), lose_noise.unsqueeze(0)])
                    ...
                    valid_noise_added.append(
                        noise_sam[valid_samples.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(
                        2, -1, *collected_valid_next_latents.shape[3:]
                    )].reshape(2, -1, *collected_valid_next_latents.shape[3:]))
                    ...
                    prev_latents_cloned = prev_latents.clone()
                    ...
                    noise_temp = noise_temp.reshape(num_samples_each_step, batch_temp, prev_latents_cloned.shape[1], prev_latents_cloned.shape[2], prev_latents_cloned.shape[2])
                    idx_max = preference_scores.max(dim=0)[1]
                    idx_range = torch.arange(preference_scores.shape[1]).to(preference_scores.device)
                    idx_min = preference_scores.min(dim=0)[1]
                    max_noise = noise_temp[idx_max, idx_range]
                    min_noise = noise_temp[idx_min, idx_range]
                    win_noise = (max_noise-min_noise)/2
                    lose_noise = -win_noise.to(latents.dtype)
                    ...
                    eta = extra_step_kwargs['eta']
                    noise_sam = torch.cat([win_noise.unsqueeze(0), lose_noise.unsqueeze(0)])
                    del next_latents_cache[-1]
                    del timestep_cache[-1]
                    del current_latents_cache[-1]
                    # indices: 2,b
                    # valid_samples: b
                    indices, valid_samples = compare_fn(preference_scores)
                ...
                latents, noise_temp = ddim_step_fetch_x_t_1(
                    self.scheduler,
                    dtype=latents.dtype,
                    num_sample_per_step=1,
                    timestep=t,
    ...
    valid_noise_added = torch.cat(valid_noise_added, dim=1).transpose(0, 1).contiguous()
