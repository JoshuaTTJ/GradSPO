...
def multi_sample_pipeline(
    ...
    **kwargs,
        ...
        for i, t in enumerate(timesteps):
            ...
            if i >= divert_start_step:
                ...
                if i == divert_start_step:
                    ...
                    valid_noise_added.append(torch.gather(
                        noise_temp, 
                        dim=0, 
                        index=indices.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(-1, -1, *noise_temp.shape[2:]),
                    )[valid_samples.unsqueeze(0).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(
                        2, -1, *all_prev_latents.shape[2:]
                    )].reshape(2, -1, *all_prev_latents.shape[2:]))
                    
                    valid_noise_sam.append(
                ...
                if i == len(timesteps) - 1:
                    ...
                    preference_timestep = t.repeat(prev_latents.shape[0])
                    extra_info['timesteps'] = preference_timestep
                    # num_sample_per_step*b
                    # preference scores of i-1
    ...
    valid_noise_sam = torch.cat(valid_noise_sam, dim=1).transpose(0, 1).contiguous()
    # valid_num,1,l,c
    valid_prompt_embeds = torch.cat(valid_prompt_embeds, dim=0)
    
    preference_score_logs = torch.cat(preference_score_logs, dim=0)
    return valid_timesteps, valid_current_latents, valid_next_latents, valid_noise_added, valid_noise_sam, valid_prompt_embeds, preference_score_logs
