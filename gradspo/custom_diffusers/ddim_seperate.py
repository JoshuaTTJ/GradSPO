...
def ddim_step_fetch_x_t_1(
    ...
    if return_noise:
        return prev_sample.type(dtype), variance_noise
    else:
        return prev_sample.type(dtype)

def ddim_inv_step_fetch_x_t(
    self: DDIMScheduler,
    latents,
    pred_epsilon,
    timestep,
    ...
    # 2. compute alphas, betas
    alpha_prod_t_cur = self.alphas_cumprod.gather(0, timestep)
    alpha_prod_t_prev = torch.where(
        prev_timestep >= 0, self.alphas_cumprod.gather(0, prev_timestep), self.final_alpha_cumprod
    )
    alpha_prod_t_cur = _left_broadcast(alpha_prod_t_cur, pred_epsilon.shape)
    alpha_prod_t_prev = _left_broadcast(alpha_prod_t_prev, pred_epsilon.shape)
    pred_x0 = (latents - ((1-alpha_prod_t_cur)**0.5)*pred_epsilon)/alpha_prod_t_cur**0.5
    prev_sample = (alpha_prod_t_prev**0.5)*pred_x0 + ((1-alpha_prod_t_prev)**0.5)*pred_epsilon  
    return prev_sample.type(dtype)

def cfgpp_inv_step_fetch_x_t(
    self: DDIMScheduler,
    latents,
    pred_epsilon,
    timestep,
...
def cfgpp_fetch_x_t(
    ...
    alpha_prod_t_prev = _left_broadcast(alpha_prod_t_prev, pred_epsilon.shape)
    pred_x0 = (latents - ((1-alpha_prod_t_cur)**0.5)*pred_epsilon)/alpha_prod_t_cur**0.5
    prev_sample = (alpha_prod_t_prev**0.5)*pred_x0 + ((1-alpha_prod_t_prev)**0.5)*pred_epsilon_null  
    return prev_sample.type(dtype)
...
def cfgpp_inv(
    ...
    alpha_prod_t_cur = _left_broadcast(alpha_prod_t_cur, pred_epsilon.shape)
    alpha_prod_t_prev = _left_broadcast(alpha_prod_t_prev, pred_epsilon.shape)
    pred_x0 = (latents - ((1-alpha_prod_t_cur)**0.5)*pred_epsilon_null)/alpha_prod_t_cur**0.5
    prev_sample = (alpha_prod_t_prev**0.5)*pred_x0 + ((1-alpha_prod_t_prev)**0.5)*pred_epsilon  
    return prev_sample.type(dtype)