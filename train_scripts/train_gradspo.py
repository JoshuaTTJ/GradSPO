...
from gradspo.preference_models import get_preference_model_func, get_compare_func
from gradspo.datasets import build_dataset
from gradspo.utils import (
    huggingface_cache_dir, 
    UNET_CKPT_NAME, 
    gather_tensor_with_diff_shape,
)
from gradspo.custom_diffusers import (
...
config_flags.DEFINE_config_file(
    "config", 
    "configs/gradspo_sd-v1-5_4k-prompts_num-sam-4_10ep_bs10.py", 
...
def main(_):
    ...
    ema_model = copy.deepcopy(accelerator.unwrap_model(unet)).eval()
    ema_model.to(accelerator.device)
    ema_model.requires_grad_(False)
    pipeline.unet = ema_model
    ...
    for epoch in tqdm(
        ...
        for dataset_batch_idx, batch in tqdm(
            ...
            if accelerator.num_processes > 1:
                ...
                noise_added = gather_tensor_with_diff_shape(noise_added, local_valid_samples_num_list)
                noise_sam = gather_tensor_with_diff_shape(noise_sam, local_valid_samples_num_list)
                ...
                with accelerator.accumulate(unet):
                    ...
                    with autocast():
                        if config.train.cfg:
                            ...
                            noise_ema_pred_0 = ema_model(
                                torch.cat([sample_0["latents"][train_b_start: train_b_end]] * 2),
                                torch.cat([sample_0["timesteps"][train_b_start: train_b_end]] * 2),
                                embeds_0,
                            ).sample

                            noise_ema_pred_uncond_0, noise_ema_pred_text_0 = noise_ema_pred_0.chunk(2)
                            noise_ema_pred_0 = noise_ema_pred_uncond_0 + config.sample.guidance_scale * (
                                noise_ema_pred_text_0 - noise_ema_pred_uncond_0
                            )
 
                            noise_pred_1 = noise_pred_0
                            noise_ref_pred_1 = noise_ref_pred_0
                            noise_ema_pred_1 = noise_ema_pred_0
                    ...
                    loss_0 = score_matching_loss(
                        pipeline.scheduler,
                        noise_pred_0,
                        sample_0["timesteps"][train_b_start: train_b_end],
                        noise_added=sample_0["noise_sam"][train_b_start: train_b_end],
                        ema_noise=noise_ema_pred_0,
                        grad_scale=config.train.grad_scale
                    )
                    loss_ref_0 = score_matching_loss(
                        pipeline.scheduler,
                        noise_ref_pred_0,
                        sample_0["timesteps"][train_b_start: train_b_end],
                        noise_added=sample_0["noise_sam"][train_b_start: train_b_end],
                    ...
                    loss_1 = score_matching_loss(
                        pipeline.scheduler,
                        noise_pred_1,
                        sample_1["timesteps"][train_b_start: train_b_end],
                        noise_added=sample_1["noise_sam"][train_b_start: train_b_end],
                        ema_noise=noise_ema_pred_1,
                        grad_scale=config.train.grad_scale
                    )
                    loss_ref_1 = score_matching_loss(
                        pipeline.scheduler,
                        noise_ref_pred_1,
                        sample_1["timesteps"][train_b_start: train_b_end],
                        noise_added=sample_1["noise_sam"][train_b_start: train_b_end],
                    ...
                    # # clip the Q value
                    ratio_0 = torch.clamp(torch.exp(loss_0-loss_ref_0),1 - config.train.eps, 1 + config.train.eps)
                    ratio_1 = torch.clamp(torch.exp(loss_1-loss_ref_1),1 - config.train.eps, 1 + config.train.eps)
                    ...
                    win_prob_sum =  accelerator.reduce(loss_0.detach(), reduction='sum')   
                    ...
                    avg_win_total_prob = (win_prob_sum.sum() / (win_prob_sum.shape[0] * accelerator.num_processes)).item()
                    ...
                    train_win_total_prob += avg_win_total_prob / accelerator.gradient_accumulation_steps
                    ...
                    optimizer.step()
                    optimizer.zero_grad()
                    if (train_batch_idx + 1) % accelerator.gradient_accumulation_steps == 0:
                        update_ema(ema_model, accelerator.unwrap_model(unet), decay=config.train.ema_decay)
            ...
            if accelerator.sync_gradients:
                ...
                info = {
                    ...
                    "total_win_prob": train_win_total_prob
                }
